...
void toLaTeX(const char *fname = "pt_RFE")
{
  TTree *t = new TTree();
  t->ReadFile(fname, "x:y:dx:dy:dy2"); // default read as float

  t->SetBranchAddress("x", &x);
  t->SetBranchAddress("y", &y);
  t->SetBranchAddress("dx", &dx);
  t->SetBranchAddress("dy", &dy);
  t->SetBranchAddress("dy2", &dy2);
...
void PrintHeader()
{
  ...
  outfile << "  \\hline"<< endl;
  outfile << "  $p_{T, min}$ & $p_{T, max}$ & value & stat. & sys. \\\\ \\hline"
          << endl;
  outfile << "  \\hline" << endl;
}
